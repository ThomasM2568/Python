...
def case():
    global nbcle,player,brouillarddefinitif,fenetre,labelcle
    canvas.delete(player)
    if brouillard.get()==0 and deplacements==0:
        affichage_labsansbrouill()
    if brouillard.get()==1:
        brouillarddefinitif=1
        canvas.delete(ALL)
        affichage_labavecbrouill()
    if lines[posy][posx]=='3':
        lines[posy][posx]='0'
        nbcle+=1
        labelcle=Label(fenetre,text=('Nombre de cles:',nbcle))
        labelcle.grid_forget()
        labelcle.grid(row=1,column=2)
...
portailphoto = PhotoImage(file="Loop_Portal.gif")
...
def generation():
    global canvas,fenetre,gene,posx, posy,deplacements, nbcle,labelcle,lines
    ...
    for i in range (len (lines)):
        for j in range (len(lines[0])):
            if lines[i][j]=='0':
                        ...
                        canvas.create_image(i*13,j*13,anchor=NW, image=portailphoto, tag='portailphoto')
    player=canvas.create_image(posx*13+1,posy*13+1,anchor=NW,image=joueurphoto)
    update()
...
lab1=Radiobutton(fenetre1,text="13x54",variable=aleatoire,value=1,bg='#AAAAAA')
...
lab4=Radiobutton(fenetre1,text="15x15",variable=aleatoire,value=4,bg='#AAAAAA')
...
fenetre1.title("Bienvenue dans le labyrinthe, selectionnez un niveau")
fenetre1.mainloop()
